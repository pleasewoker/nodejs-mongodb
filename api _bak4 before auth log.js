...
organizationRouter.get('/', async (req, res, next) => {
  try {
    ...
    return ok(res, 'Get organizations successful', rows);
  } catch (e) { next(e); }
});
...
// LIST CODE
// =========================
organizationRouter.get('/code', async (req, res, next) => {
  try {
    const [rows] = await pool.execute(
      `SELECT 
        email,
    ...
    return ok(res, 'Get organizations-code successful', rows);
  } catch (e) { next(e); }
});
...
organizationRouter.get('/:id', async (req, res, next) => {
  try {
    ...
    return ok(res, 'Get organizations-id successful', rows[0]);
  } catch (e) { next(e); }
});
...
userRouter.get('/', async (req, res, next) => {
  try {
    const [rows] = await pool.execute(
      `SELECT user_id, organization_id, group_id, email, full_name, license, picture, active_status, created_date, updated_date
       FROM m_user
...
userRouter.get('/:id', async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT user_id, organization_id, group_id, email, full_name, phone_number, license, picture, active_status, created_date, updated_date
...
userRouter.post('/', async (req, res, next) => {
  try {
    const {
      ...
      email, full_name, phone_number, license, password,
    ...
    const [result] = await pool.execute(
      `INSERT INTO m_user (organization_id, group_id, email, full_name, phone_number, license, password, picture, active_status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
    ...
    const [rows] = await pool.execute(
      `SELECT user_id, organization_id, group_id, email, phone_number, full_name, license, picture, active_status, created_date, updated_date
       FROM m_user WHERE user_id = ? LIMIT 1`,
      [result.insertId]
    );
...
userRouter.patch('/:id', async (req, res, next) => {
  try {
    ...
    const allow = ['organization_id','group_id','email','phone_number','full_name','license','password','picture','active_status'];
    for (const k of allow) {
      if (k in (req.body || {})) {
        if (k === 'password') {
          const hash = await bcrypt.hash(req.body.password, SALT_ROUNDS);
...
userRouter.put('/:id', async (req, res, next) => {
  try {
    ...
    await pool.execute(
      `UPDATE m_user
       SET organization_id = ?, group_id = ?, email = ?, phone_number = ?, full_name = ?, license = ?, password = ?, picture = ?, active_status = ?
       WHERE user_id = ?`,
      [
        organization_id ?? null,
        group_id ?? null,
...
// =========================
// COMPLETE PROFILE (USER) by email
// ใช้หลัง verify-email แล้วไปหน้า register เพื่อกรอก full_name, phone_number
// =========================
app.post('/auth/user/complete-profile', async (req, res, next) => {
  try {
    const { email, full_name, phone_number } = req.body || {};

    if (!email) return fail(res, 'email is required', 400);
    if (!full_name) return fail(res, 'full_name is required', 400);
    if (!phone_number) return fail(res, 'phone_number is required', 400);

    const emailNorm = String(email).trim().toLowerCase();
    const fullNameNorm = String(full_name).trim();
    const phoneNorm = String(phone_number).trim();

    // ตรวจสอบว่ามี user และยังไม่เคยกรอก full_name
    const [exist] = await pool.execute(
    ...
    if (exist[0].full_name !== null) {
      return fail(res, 'Profile already completed', 409);
    }

    // Update เฉพาะกรณี full_name ยังเป็น NULL เท่านั้น
    const [result] = await pool.execute(
      `UPDATE m_user
       SET full_name = ?, phone_number = ?, updated_date = NOW()
       WHERE email = ?
       AND full_name IS NULL`,
      [fullNameNorm, phoneNorm, emailNorm]
    );
    ...
    return ok(res, 'Complete profile successful', rows[0]);
...
app.post('/auth/user/login', async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT user_id, email, full_name, phone_number, password, organization_id, group_id, license, active_status
       FROM m_user
       WHERE email = ? LIMIT 1`,
      [email]
    );
